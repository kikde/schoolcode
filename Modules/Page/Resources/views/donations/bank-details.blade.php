                  ...
                  <form action="{{ url('/bank-details/'.$list->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="modal-body">
                      ...
                      <div class="form-group">
                        @php
                          $msgRaw = $list->message;
                          $msgArr = is_string($msgRaw) ? json_decode($msgRaw, true) : null;
                          $messageValue = is_array($msgArr) ? ($msgArr['text'] ?? '') : $msgRaw;
                        @endphp
                        <textarea name="message" class="form-control">{{ $messageValue }}</textarea>
                      </div>
                      <label>UPI ID:</label>
                      <div class="form-group">
                        <input type="text" name="upi_id" value="{{ $list->upi_id }}" class="form-control" placeholder="example@upi" />
                      </div>
                      ...
                      <div class="form-group">
                        ...
                        @php
                          $qr = $list->qr_path;
                          if (empty($qr) && !empty($list->message)) {
                            $tmp = json_decode($list->message, true);
                            if (is_array($tmp) && !empty($tmp['qr'])) $qr = $tmp['qr'];
                          }
                        @endphp
                        @if(!empty($qr))
                          @php
