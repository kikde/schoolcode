...
class SettingController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Secondary header logo (header_logo2)
        if ($request->hasFile('header_logo2')) {
            $postimage = $request->file('header_logo2');
            $filename = ($request->name ?? 'header_logo2') . '_prd_'. time() . '.' . $postimage->getClientOriginalExtension();
            Image::make($postimage)->resize(600, 500, function ($constraint) {
                $constraint->aspectRatio();
            })->save(public_path('/backend/uploads/'. $filename));
            $set->header_logo2 = $filename;
        }
